var rubrics = {
  ...
  initLightbox(token) {
    ...
    var scrollTop =
      rubrics.windowRef.pageYOffset ||
      rubrics.windowRef.document.documentElement.scrollTop;
    this.appendStringAsNodes(
      rubrics.windowRef.document.body,
      '<div class="rubrics-lightbox" style="display:none"><div class="container"><a href="#" class=>&times;</a><sakai-rubric-student token="' +
        token +
        '"></sakai-rubric-student></div></div>'
    );
  ...
  showRubric(id, attributes) {
    ...
    this.css(rubrics.lightbox[0], {
      height: rubrics.windowRef.window.innerHeight + "px",
      width: rubrics.windowRef.window.innerWidth + "px",
      top: scrollTop + "px"
    });
