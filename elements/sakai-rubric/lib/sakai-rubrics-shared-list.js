...
export class SakaiRubricsSharedList extends SakaiElement {
  ...
  getSharedRubrics(token) {
    ...
    SakaiRubricsHelpers.get(
      "/rubrics-service/rest/rubrics/search/shared-only",
      token,
      { params }
  ...
  copyToSite(e) {
    var options = {
      extraHeaders: { "x-copy-source": e.detail, lang: portal.locale }
    };
    SakaiRubricsHelpers.post(
      "/rubrics-service/rest/rubrics/",
      this.token,
      options
