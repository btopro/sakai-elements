  ...
  render() {
    return html`
      <span
        ...
        @click="${this.editCriterion}"
        title="${tr("edit_criterion")} ${this.criterion.title}"
      ></span>

      <div
        id="edit_criterion_${this.criterion.id}"
        class="popover criterion-edit-popover bottom"
      >
        <div class="arrow"></div>
        ...
        <div class="popover-content form">
          <div class="form-group">
            ...
            <input
              id="criterion-title-field-${this.criterion.id}"
              type="text"
              class="form-control"
          ...
          <div class="form-group">
            <label>
              <sr-lang key="criterion_description"
                >Criterion Description</sr-lang
              >
            </label>
            <textarea
              ...
              class="form-control"
            >
${this.criterionClone.description}</textarea
            >
          </div>
        </div>
      </div>
  ...
  editCriterion(e) {
    ...
    this.dispatchEvent(
      new CustomEvent("show-tooltip", { detail: this.criterion })
    );
  ...
  hideToolTip() {
    ...
    this.dispatchEvent(
      new CustomEvent("hide-tooltip", {
        details: { criterion: this.criterion }
      })
    );
  }
  ...
  cancelEdit(e) {
    ...
    this.hideToolTip();
    this.dispatchEvent(
      new CustomEvent("hide-tooltip", { details: this.criterion })
    );
    $(`#edit_criterion_${this.criterion.id}`).hide();
  }
  ...
  saveEdit(e) {
    ...
    var data = {
      title: document.getElementById(
        `criterion-title-field-${this.criterion.id}`
      ).value,
      description: document.getElementById(
        `criterion-description-field-${this.criterion.id}`
      ).value
    };
    ...
    this.hideToolTip();
    this.dispatchEvent(
      new CustomEvent("hide-tooltip", { detail: this.criterion })
    );
    $(`#edit_criterion_${this.criterion.id}`).hide();
  }
  ...
  updateUi(data) {
    this.hideToolTip();
    this.dispatchEvent(
      new CustomEvent("criterion-edited", {
        detail: {
          id: data.id,
          title: data.title,
