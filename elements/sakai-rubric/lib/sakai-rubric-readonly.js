...
export class SakaiRubricReadonly extends SakaiElement {
    ...
    return html`
      <div class="rubric-title" @click="${this.toggleRubric}">
        <div>
          <span
            class="rubric-name"
            id="rubric_toggle_shared_${this.rubric.id}"
        ...
        <div class="actions">
          <div class="action-container">
            ...
            <span
              role="button"
              title="${tr("copy_to_site", [this.rubric.title])}"
              tabindex="0"
              class="clone fa fa-copy"
              @click="${this.copyToSite}"
            ></span>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        id="collapse_shared_${this.rubric.id}"
      >
  ...
  cloneRubric(e) {
    e.stopPropagation();
    this.dispatchEvent(
      new CustomEvent("clone-rubric", { detail: this.rubric })
    );
  }
  ...
  copyToSite(e) {
    e.stopPropagation();
    this.dispatchEvent(
      new CustomEvent("copy-to-site", { detail: this.rubric.id })
    );
  }
}
