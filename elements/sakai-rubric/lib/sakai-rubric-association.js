...
class SakaiRubricAssociation extends SakaiElement {
  constructor() {
    ...
    SakaiRubricsLanguage.loadTranslations().then(
        ...
        ${this.rubrics && this.rubrics.length
          ? html`
              <div class="radio">
                <label>
                  <input
                    ...
                    id="rbcs-dontAssociate"
                    name="rbcs-associate"
                    type="radio"
                    value="${this.dontAssociateValue}"
                  />${this.dontAssociateLabel}
                </label>
              </div>
              ...
              <div class="rubrics-list ${this.enabledClass}">
                <div class="rubrics-selections">
                  <select
                    id="rbcs-rubricslist"
                    @change="${this.updateStateDetails}"
                  ...
                  <button
                    @click="${this.showRubric}"
                    id="preview-rubric"
                    class="btn btn-link ${this.enabledClass}"
                  >
                ...
                <div class="rubric-options">
                  ...
                  <div class="checkbox">
                    <label>
                      <input
                        @change="${this.updateStateDetails}"
                        id="rbcs-config-hideStudentPreview"
                        name="rbcs-config-hideStudentPreview"
                        type="checkbox"
                        value="1"
          ...
          : html`
              <span class="messageInformation indnt2" style="width: 80%">
                <sr-lang key="no_rubrics"
                  >No rubrics have been created.</sr-lang
                >
              </span>
            `}
      </div>
      <input
        name="rbcs-state-details"
        id="rbcs-state-details"
        type="hidden"
        value="${this.stateDetails}"
      />
    `;
  }
  ...
  getAssociation() {
    $.ajax({
      url: `/rubrics-service/rest/rubric-associations/search/by-tool-item-ids?toolId=${
        this.toolId
      ...
      setTimeout(
        function() {
          ...
          if (this.selectedRubric) {
            this.querySelector(
              `option[value="${this.selectedRubric}"]`
            ).selected = true;
  ...
  handleOptions() {
    this.selectedConfigOptions = this.association.parameters
      ? this.association.parameters
      : {};

    for (var property in this.selectedConfigOptions) {
      document.getElementById(
        `rbcs-config-${property}`
      ).checked = this.selectedConfigOptions[property];
    }
  }
    ...
    if (this.isAssociated == 1) {
      ...
      setTimeout(
        function() {
          document.getElementById("rbcs-associate").checked = true;
          document
            .getElementById("rbcs-rubricslist")
          ...
          this.querySelectorAll(
            ".rubric-options input[type='checkbox']"
