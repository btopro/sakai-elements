  ...
  render() {
    return html`
      <span
        ...
        @click="${this.editRating}"
        title="${tr("edit_rating")} ${this.rating.title}"
      ></span>

      <div
        id="edit_criterion_rating_${this.rating.id}"
        class="popover rating-edit-popover bottom"
      >
        <div class="arrow"></div>
        <div class="popover-title">
          <div class="buttons">
            <button
              ...
              title="${tr("save")} ${this.rating.title}"
              @click="${this.saveEdit}"
            >
            ...
            <button
              class="delete"
              title="${tr("remove_label")} ${this.rating.title}"
              @click="${this.deleteRating}"
            >
              <sr-lang key="remove_label" />
            </button>
            <button
              class="btn btn-link btn-xs cancel"
              title="${tr("cancel")}"
        ...
        <div class="popover-content form">
          ...
          <div class="form-group">
            ...
            <textarea
              name=""
              id="rating-description-${this.rating.id}"
              class="form-control"
            >
${this.rating.description}</textarea
            >
          </div>
        </div>
      </div>
  ...
  resetFields() {
    document.getElementById(
      `rating-title-${this.rating.id}`
    ).value = this.rating.title;
    document.getElementById(
      `rating-points-${this.rating.id}`
    ).value = this.rating.points;
    document.getElementById(
      `rating-description-${this.rating.id}`
    ).value = this.rating.description;
  }
  ...
  saveEdit(e) {
    ...
    this.rating.title = document.getElementById(
      `rating-title-${this.rating.id}`
    ).value;
    this.rating.points = document.getElementById(
      `rating-points-${this.rating.id}`
    ).value;
    this.rating.description = document.getElementById(
      `rating-description-${this.rating.id}`
    ).value;
  ...
  deleteRating(e) {
    ...
    this.rating.criterionId = this.criterionId;
    this.dispatchEvent(
      new CustomEvent("delete-rating", { detail: this.rating })
    );
    this.hideToolTip();
  }
}
