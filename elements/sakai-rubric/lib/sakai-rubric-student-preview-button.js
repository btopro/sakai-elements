...
export class SakaiRubricStudentPreviewButton extends SakaiElement {
  ...
  render() {
    return html`
      ${this.display === "button"
        ? html`
            <h3><sr-lang key="grading_rubric" /></h3>
            <button @click="${this.showRubric}">
              <sr-lang key="preview_rubric" />
            </button>
          `
        : html`
            <span
  ...
  getRubricId() {
    SakaiRubricsHelpers.get(
      "/rubrics-service/rest/rubric-associations/search/by-tool-item-ids",
      this.token,
