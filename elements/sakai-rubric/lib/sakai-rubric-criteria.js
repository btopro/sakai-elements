...
import * as Unused from "./assets/sortablejs/Sortable.js";
...
export class SakaiRubricCriteria extends SakaiElement {
  ...
  updated(changedProperties) {
    ...
    this.querySelectorAll(".cr-table").forEach(
      cr => new Sortable(cr, sortableOptions)
    );

    sortableOptions.onUpdate = e => this.handleSortedCriteria(e);
    sortableOptions.draggable = ".criterion-row";

    this.querySelectorAll(".criterion").forEach(
      c => new Sortable(c, sortableOptions)
    );
  ...
  render() {
    return html`
      <div
        data-rubric-id="${this.rubricId}"
        class="criterion style-scope sakai-rubric-criterion"
      >
        ${repeat(
          this.criteria,
          c => c.id,
            ...
            >
                    ...
                    (r, i) => html`
                      ...
                      >
                        ...
                        <span
                          ...
                          role="button"
                          title="${tr("drag_order")}"
                          class="reorder-icon sideways fa fa-bars"
                        ></span>
                      </div>
                    `
                  )}
              ...
              <div class="criterion-actions">
                <span
                  ...
                  role="button"
                  data-criterion-id="${c.id}"
                  title="${tr("copy")} ${c.title}"
                  class="clone fa fa-copy"
                ...
                <sakai-item-delete
                  ...
                  @delete-item="${this.deleteCriterion}"
                  token="${this.token}"
                ></sakai-item-delete>
              </div>
            </div>
  ...
  handleSortedCriteria(e) {
    ...
    }/rubrics-service/rest/criterions/`;
    var sortedIds = Array.from(this.querySelectorAll(".criterion-row")).map(
      c => c.dataset.criterionId
    );
    const urlList = sortedIds.reverse().reduce((a, cid) => {
      return `${baseUrl}${cid}\n${a}`;
    }, "");
  ...
  handleSortedCriterionRatings(e) {
    ...
    var sortedIds = Array.from(
      this.querySelectorAll(`#cr-table-${criterionId} .rating-item`)
    ).map(r => r.dataset.ratingId);

    // Reorder the ratings based on the sort result
    criterion.ratings = sortedIds.map(id =>
      criterion.ratings.find(r => r.id == id)
    );

    var baseUrl = `${window.location.protocol}//${
      window.location.host
    }/rubrics-service/rest/ratings/`;
    const urlList = sortedIds.reverse().reduce((a, rid) => {
      return `${baseUrl}${rid}\n${a}`;
    }, "");
  ...
  addRatingResponse(criterionId, ratingPos, newRating) {
    ...
    var baseUrl =
      getUrl.protocol + "//" + getUrl.host + "/rubrics-service/rest/ratings/";
  ...
  deleteRating(e) {
    ...
    const urlList = criterion.ratings
      .slice()
      .reverse()
      .reduce((a, v) => {
        return `${baseUrl}${v.id}\n${a}`;
    ...
    var url = `/rubrics-service/rest/criterions/${
      e.detail.criterionId
    }/ratings`;
    this.updateRatings(url, urlList);
  }
  ...
  cloneCriterion(e) {
    ...
    $.ajax({
      ...
      headers: {
        "x-copy-source": e.target.dataset.criterionId,
        authorization: this.token,
        lang: portal.locale
      },
      contentType: "application/json",
  ...
  criterionEdited(e) {
    ...
    var sakaiItemDelete = this.querySelector(
      `sakai-item-delete[criterion-id="${e.detail.id}"]`
    );
