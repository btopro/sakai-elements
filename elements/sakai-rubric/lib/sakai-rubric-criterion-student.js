...
export class SakaiRubricCriterionStudent extends SakaiElement {
  ...
  render() {
    return html`
      <div
        class="criterion grading style-scope sakai-rubric-criterion-student"
                ...
                <div class="cr-table">
                  <div class="cr-table-row">
                    ${repeat(
                      ...
                      r => html`
                        <div
                          class="rating-item student ${r.selected
                            ? "selected"
                            : ""}"
                          id="rating-item-${r.id}"
                        >
                ...
                ${!this.preview
                  ? html`
                      ...
                      ${this.isOverridden(c.pointoverride, c.selectedvalue)
                        ? html`
                            <strong class="points-display"
                              >${c.pointoverride}</strong
                            >
                          `
                        : html``}
                    `
        ...
        ? html`
            <div class="rubric-totals" style="margin-bottom: 5px;">
              <div class="total-points">
                Total: <strong>${this.totalPoints}</strong>
              </div>
            </div>
          `
        : html``}
  ...
  isOverridden(pointoverride, selected) {
    ...
    if (
      (pointoverride || pointoverride === 0) &&
      parseInt(pointoverride) !== parseInt(selected)
    ) {
      return true;
    } else {
      return false;
