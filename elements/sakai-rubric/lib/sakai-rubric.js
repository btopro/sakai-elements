...
export class SakaiRubric extends SakaiElement {
  ...
  set rubric(newValue) {
    ...
    this.updateRubricConfig.url = `/rubrics-service/rest/rubrics/${
      newValue.id
  ...
  render() {
    return html`
      <div class="rubric-title" @click="${this.toggleRubric}">
        ...
        <div class="hidden-xs">
          <site-title site-id="${this.rubric.metadata.ownerId}"></site-title>
        </div>
        <div class="hidden-xs">
          <creator-name
            creator-id="${this.rubric.metadata.creatorId}"
          ></creator-name>
        </div>
        <div class="hidden-xs">
          <modified-date
            modified="${this.rubric.metadata.modified}"
          ></modified-date>
        </div>
          ...
          <div class="action-container">
            <span class="hidden-sm hidden-xs sr-only">
              ${this.rubric.metadata.public
                ? html`
                    <sr-lang key="revoke_label">revoke_label</sr-lang>
                  `
                : html`
                    <sr-lang key="share_label">share_label</sr-lang>
                  `}
            </span>
            <span
              role="button"
              title="${tr(this.shareTitleKey, [this.shareValues])}"
              tabindex="0"
              class="share fa ${this.shareIcon}"
      ...
      <div
        ...
        role="tabpanel"
        aria-labelledby="rubric_toggle_${this.rubric.id}"
        id="collapse_${this.rubric.id}"
      >
  ...
  deactivate(e, id) {
    if (
      !document.getElementById(`collapse_${this.rubric.id}`).opened &&
      this.rubric.id !== id
    ) {
  ...
  updateRubricTitle(e) {
    this.updateRubricConfig.data = JSON.stringify([
      { op: "replace", path: "/title", value: e.detail }
    ]);
    this.updateRubric();
  }
  ...
  sharingChange(e) {
    ...
    this.updateRubricConfig.data = JSON.stringify([
      {
        op: "replace",
        path: "/metadata/shared",
        value: this.rubric.metadata.public
      }
    ]);
    this.updateRubric();
  }
